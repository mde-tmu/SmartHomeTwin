...
public class DataPlotter extends DashboardFrame_Editor {
   ...
   public static void PowerConsumptionDataFromStatechart(double total, double SF, double STV, double SMW, double SGD, double SL) {
       if(SF_status)
       ...
       if(SGD_status)
           DataPlotter.PieDataset.setValue( "Smart Garage Door System" , (double)SGD/(double)total*100 );
      else
           DataPlotter.PieDataset.setValue( "Smart Garage Door System" , -1 );
       
       ArrayList<String> PieChartReport = new ArrayList();
       
       PieChartReport.add("Power Distribution [SF, STV, SL, SMW, SGD] :"+"["+(double)SF/(double)total*100+", "
       +(double)STV/(double)total*100+", "
		+(double)SL/(double)total*100+", "
		+(double)SMW/(double)total*100+", "
		+(double)SGD/(double)total*100+"]\n");
   ...
   private static PieDataset<String> createDataset( ) {
	   ...
	   PieDataset.setValue( "Smart Garage Door System" , 0 ); 
   ...
   public static void runPlotter() {
      ...
      powerDistributionLevels.setSize( 560 , 367 );
       UIUtils.centerFrameOnScreen( powerDistributionLevels );
   ...
   public static CategoryDataset FireAlarmDataset(double sensor1_val, double sensor2_val, double sensor3_val) {
       ...
       dataset.setValue(sensor1_val, sensor1, "Flame");
